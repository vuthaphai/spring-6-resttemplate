...
public class BeerDTOPageDeserializer extends JsonDeserializer<BeerDTOPageImpl<BeerDTO>> {
    ...
    public BeerDTOPageImpl<BeerDTO> deserialize(JsonParser p, DeserializationContext ctxt)
        ...
        if (contentNode == null) {
            throw new JsonProcessingException("Missing 'content' node in JSON") {};
        }

        List<BeerDTO> beers = new ArrayList<>();
        for (JsonNode beerNode : contentNode) {
        ...
        int size = pageableNode.has("pageSize") && !pageableNode.get("pageSize").isNull() ? pageableNode.get("pageSize").asInt() : 1;
